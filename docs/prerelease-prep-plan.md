...
## Follow-through: complete guidance and contextual practice (22 September)

All 280 nonbasic FRA cards now have short editorial guidance anchored to both full written reviews and canonical rules. The 24-card starting selection remains deliberately finite; All cards provides the rest. Ten authored situations add support checks, resource tradeoffs, sideboarding, removal dependencies and timing, with explanations for each choice. These live in the existing final preparation lesson, not in the rating-training tab.

A set-specific review list is available above the lessons. Cards, combinations, missed rules/situation questions and Card memory misses can feed it. Opening an item returns to the relevant card, combination or question; explicit “Done reviewing” removes it. Answers and reading completion remain separate from reminders and are not treated as mastery. Existing progress namespaces are preserved.
