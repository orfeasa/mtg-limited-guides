...
When `earlyEvidence.teaching` is present, Prerelease prep presents five lessons: mechanics, priority cards, connections/colour plans, opposing interaction, and practice/checklist. It keeps the established set and format gates; Two-Headed Giant retains its own guide. `prep-journey.js` stores self-reported completion and review flags independently of answers and deck checklists, per set, teaching revision and format. Navigating or reading does not award completion. Incoming section anchors open the matching lesson.
