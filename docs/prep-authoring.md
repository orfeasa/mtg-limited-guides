...
- Preparation content teaches concepts before an event. Do not add deck checklists, manual completion marks, saved review lists, or controls intended for use during a game.
...
When `earlyEvidence.teaching` is present, Prerelease prep presents five lessons: mechanics, priority cards, connections/colour plans, opposing interaction, and practice. It keeps the established set and format gates; Two-Headed Giant retains its own guide. `prep-journey.js` provides five freely browsable lessons with Previous/Next controls. It does not store reading progress or ask users to mark completion. Incoming section anchors open the matching lesson.

Author concise teaching in `data/research/<set>/teaching/cards.json`, with both original reviewer section/paragraph references and a fingerprint of the rules/review basis. A role, strength, support condition and downside are required. Describe disagreements only after reading the prose; do not infer them from grades on different scales. Full reviews remain available for cards without an authored takeaway. Keep article counts separate from independent reviewers, mark cycle assessments, and preserve the distinction between Limited opinion and Sealed results.
...
For additive guide updates that preserve all existing question IDs and answers, retain `progressRevision` while advancing `authoredAt`. Changed meanings must advance the progress revision. The September 22 update keeps the September 20 namespace so existing answers survive the three added questions.


All formats omit deck checklists. Manual lesson completion and personal review lists have also been removed: the site is for preparation, and browser-local reading marks do not follow users across platforms. Old checklist anchors open practice. Actual quiz answers and study runs retain their existing namespaces; no unrelated stored progress is cleared. Interaction filters describe the cards being studied rather than a current opponent.
