...
(() => {
  ...
  function create(root, actions) {
    ...
    function updateChecklist() {
      if (!root.querySelector("#prep-check-count")) return;
    ...
    function render(nextSet, nextFormat) {
      ...
      root.innerHTML = `
        ...
        ${set.earlyEvidence?.teaching ? `<section id="prep-journey" class="prep-journey" aria-label="Your preparation path"></section>` : set.archetypes && window.SET_LIFECYCLE.resolve(set).memory ? `<section class="prep-study-path" aria-labelledby="prep-path-title"><h3 id="prep-path-title">Recommended preparation</h3><ol><li><a href="?set=${encodeURIComponent(set.id)}&view=archetypes&format=${format}&study=1">Archetypes</a></li><li><a href="?set=${encodeURIComponent(set.id)}&view=memory&format=${format}&studySet=essentials">Card memory</a></li><li><a href="#prep-play-around">What to play around</a></li><li><a href="#prep-practice">Quick practice</a></li></ol></section>` : ""}
        ${set.earlyEvidence?.teaching ? '<section id="prep-review-queue" aria-label="Your review list"></section>' : ""}
        <nav class="prep-jumps" aria-label="In this preparation guide" ${set.earlyEvidence?.teaching ? "hidden" : ""}><a href="#prep-mechanics">Mechanics</a><a href="#prep-cards">Cards to know</a>${set.earlyEvidence ? '<a href="#prep-combinations">Combinations</a>' : ""}<a href="#prep-play-around">Play around</a><a href="#prep-colours">Find your colours</a><a href="#prep-practice">Quick practice</a></nav>
        ...
        `}
        ${team ? `<section id="prep-checklist" class="prep-section" aria-labelledby="prep-check-title"><h3 id="prep-check-title">Build two decks together</h3><p id="prep-check-count" role="status"></p><div class="prep-checks">${guide.checklist.map((item) => `<label><input type="checkbox" data-prep-check="${item.id}" ${state.checked.includes(item.id) ? "checked" : ""}><span>${text(item.text)}</span></label>`).join("")}</div><button type="button" id="prep-reset-checks" class="prep-retry">Clear this checklist</button></section>` : ""}
