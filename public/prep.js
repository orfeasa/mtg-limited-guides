...
(() => {
  ...
  function create(root, actions) {
    ...
    function render(nextSet, nextFormat) {
      ...
      state = readState();
      root.innerHTML = `
        <header class="prep-heading"><div><h2>Prepare for your first game</h2><p>Learn the mechanics, recognise the cards that matter, and give your deck a plan.</p></div><div class="archetype-format-switch" role="group" aria-label="Preparation format"><button type="button" data-prep-format="sealed">Sealed</button><button type="button" data-prep-format="draft">Draft</button></div></header>
      ...
      renderFormat(); renderKeyCards(); renderInteractions(); updateChecklist();
    }
