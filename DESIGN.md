...
The system should feel authored, trustworthy, and ready before an event. Data maturity is visible rather than implied; readable local study images fall back safely to local thumbnails; progress and exact-tier rehearsal state stay local; rating-dependent work visibly locks when an attributable evaluation does not exist. The visual language rejects generic statistics dashboards and deck-builder framing.
...
- Evidence-gated markers: Training with complete ratings, Prerelease prep with a published guide and confirmed full file, Archetypes with an official plan map and editorial notes, and Draft decisions with grounded scenarios. The card browser stays available as Previews or All cards.
- Authored preparation uses five reading sections plus quick rules practice, with separate Sealed/Draft guidance and local checklist and answer state.
...
The system is one multi-volume field-guide shell capped at 1440px. A 164px binding leads with the selected set, subtitle, set picker, share action, and data-maturity stamp. The latest set opens by default. Sticky page markers expose supported modes: Training with complete attributable ratings, Prerelease prep with a published guide and confirmed full file, Archetypes with an attributable plan map, verified signposts, and complete editorial notes, and Draft decisions with grounded scenarios and ratings. The card browser remains available throughout. Draft and Sealed observations can arrive later as a visibly separate evidence layer. The markers sit on a 62px line, then open directly into a broad surface. The first Training viewport is one focused card-rating workspace rather than a split dashboard.

Training centres one readable card-and-answer object at a 1040px maximum, with browser-local progress inline below it. Expert review Training stacks its card and answer controls at 700px and below, centring the full card image at up to 244px wide and retaining visible labels above the four numerical-choice rows. Individual grade meanings remain in each button’s accessible name and tooltip. Archetypes uses one broad format field note, a sticky route strip, and ruled full-width chapters rather than a grid of equal cards; each chapter keeps the plan and format read beside two recognisable signposts. Draft decisions places seat and historical-pool context in ruled binding rows, the complete pack in a seven-column map-leaf grid, a staged reason-and-lock step below the pack, and the revealed evidence in a flat three-column ledger followed by a keep/change reflection. All cards uses a single broad leaf, a compact colour/tier arrangement switch for rated sets, sticky navigation for the selected grouping, an auto-filling grid with a 230px minimum, and an in-place image lightbox. Major surfaces share a continuous binding/page shadow; rows remain flat and separated by rules.

Prerelease prep extends the broad themed page with a display heading, the existing Sealed/Draft switch, a short introduction, and a wrapping ruled jump index. Five sections cover mechanics, key cards, interaction, colour selection, and a deck checklist; quick practice follows, then dated sources. Reading copy stays within 72ch, illustrated key-card rows within 850px, and checklist/practice within 76ch. Rows use the existing line and surface variables without individual elevation.

At 860px, the header becomes a grid, Training progress stacks below its heading, archetype chapters tighten their reading/signpost split, decision context becomes two columns, the pack becomes four columns, and the reasoning and evidence ledgers stack. At 700px, the binding becomes a compact mobile header, available markers remain in a sticky grid (five modes wrap across two rows), surfaces use ten-pixel outer gutters, Training remains a side-by-side card/copy object, Archetypes stacks its format ledger and each route chapter while retaining paired signposts, and Draft decisions uses a three-column pack with a two-column reason grid. Prep stacks its heading and format switch and its interaction rows; illustrated card rows retain a compact image beside the explanation, narrowing from 146px to 110px and then 92px at 390px. At 430px, the decision heading and counter stack, the pack becomes two columns, summary and reflection controls become one column, and all staged controls retain at least a 44px touch target.
...
**The Stable-Shell Rule.** Follow `docs/set-lifecycle.md`. Show only useful modes in the order Training, Prerelease prep, Archetypes, Draft decisions, card browser. Training requires complete attributable ratings; prep requires the confirmed full file and a published guide whose publication date has arrived; Archetypes requires the verified full file and authored plans. The browser is Previews while reveals are incomplete and All cards once the full file is confirmed. Default to Training, then published prep, then Archetypes, then browsing according to availability. Prep remains available after ratings arrive.
...
- **Set-Aware Page Markers:** Hide unavailable modes. Training leads with verified observed ratings or complete attributable expert grades, followed by published Prerelease prep, authored Archetypes, and optional Draft decisions with real scenarios. Previews/All cards stays last. Inactive markers use the soft binding; the selected marker rises from 50px to 58px and changes to the light metal token. Sets with either supported Training source open Training; otherwise they open Previews/All cards. A published guide adds a short “Preparing for prerelease? Start here” invitation below the card-browser heading, using the existing accent link style.
...
- **Reading Page:** The established surface, ink, display, and body variables carry the guide. Mechanics use native disclosure rows with the first example open; rules, practical tips, and linked card examples remain together. Ruled section jumps clear the sticky navigation when followed.
- **Format and Filters:** Reuse the Archetypes Sealed/Draft switch, opening Sealed by default and preserving the format in direct links and the onward Archetypes action. Role and opponent-colour selects use light surface, ink, and line variables with three-pixel corners and a 44px minimum height. The initial key-card selection introduces common/uncommon cards; role filtering exposes further selections. Colour filtering includes multicolour interaction containing that colour, with visible result counts.
- **Card References:** Underlined accent-dark card names and illustrated rows open the existing focused lightbox. Rows pair the card with printed metadata, why to know it, and its requirements. Hover changes the link to the ink variable; editorial assessments stay adjacent and muted.
- **Checklist:** Native checkboxes use the accent-dark variable in flat ruled rows, with a completion count and explicit clear action. Checks and exercise answers persist separately from Training, isolated by set, guide version/review date, and format. Clearing the checklist preserves answers; unavailable storage is explained and falls back to the page session.
- **Quick Practice:** Objective rules questions use labelled fieldsets, light-surface answer buttons, and binding/on-binding selected states. Choosing an answer reveals an announced explanation and disables choices until Try again; retries are unlimited. Existing focus treatment remains visible, with focus moving to retry after an answer and back to the first choice on retry.
- **Source Boundary:** A ruled closing source list names the guide review date and distinguishes card-text rules from editorial card selections and building advice. Prep has no rating tiers or invented observed statistics.
...
- **Focused Workspace:** One full-width themed surface holds the training heading, card, answer controls, and an inline progress ledger. The heading identifies “Training · Expert review” or “Training · Observed draft data”. There is no explanatory sidebar.
...
- **Observed Answer:** Reveal exposes exact rank, exact tier, the broader family label, and whether the call was exact, close, missed, or manually revealed. A one-step modifier difference inside the same letter grade is close and accepted without requeueing; exact-tier scoring remains exact. Correct, close, and chosen-wrong buttons remain explicit while unchosen tiers recede.
- **Expert Choices and Answer:** Reality Fracture uses J2SJosh’s original 0–5 scale in eleven numerical choices arranged in four labelled rows: premium (5–4), strong (3.5–3), solid/filler (2.5–2), and narrow/weak (1.5–0). Reuse Hobbit’s A/B/C/D row colours as visual grouping, with 44px minimum choices and visible row labels on phones. The numbers remain original grades, not converted letter tiers; individual meanings are in accessible names and tooltips. Only after answering or revealing, show the target grade, both attributed reviewers’ grades, available teaching, and expandable source context. A compact ruled Early Draft signal may follow the grades for materially discrepant cards, but it must show the metric, card sample, overall snapshot size, format, capture date, source and Draft-not-Sealed limitation. Keep original grades separate; do not replace them with the partial signal or manufacture a new grade, tier or rank.
...
- **Unavailable Evaluations:** Omit Training when neither complete observed ratings nor a complete attributable expert-grade cohort is verified. A confirmed full file and elapsed date alone do not enable Training. Published Prerelease prep, authored Archetypes, and the card browser remain available under their own gates.
- **Progress:** Cards seen, exact-tier or review-grade correct/attempt totals, current card, colour filter, shuffled queue, revealed-answer state, selected answer, and each decision's pick, reason, reveal phase, and reflection persist in browser-local storage. Expert review progress is isolated by set, reviewer group, and capture date, separate from observed Training and Card memory. A missed or manually revealed card is inserted after three intervening cards; reset is explicit and confirmed. Legacy reviewed-only markers are preserved as history but do not become fabricated completed responses.

### Draft Decisions

- **Availability:** The marker and panel are absent—not locked—when a set has no grounded pack-and-pool scenarios.
- **Fixed Replay:** State plainly that this is one historical draft. The player's choice never branches or rewrites the later pool.
- **Context First:** Pack/pick coordinate, pool direction, and the complete historical drafted pool precede the pack. The pool aggregates duplicates without erasing individual pick history from the source data.
- **Independent First:** Every available card remains visible in a true card aspect ratio, with separate View card and Choose controls. Before lock, card names and rules may be inspected, but rank, tier, performance, replay status, and data-leader hints stay out of visible and accessible metadata. Basic lands use a set-native symbolic placeholder when no local card record exists.
- **Reason Before Reveal:** After choosing a card, the player records one primary reason and explicitly locks the decision. The pick and reason are preserved as the uncontaminated pre-reveal judgement.
- **Review, Not Score:** The reveal compares the recorded decision and reason, historical replay pick, current aggregate-data leader, and authored takeaway in one flat ledger. Replay and data entries each explain why that card appears, while the comparison never marks one answer correct.
- **Reflect and Finish:** A single keep/change reflection enables navigation. The last scenario opens a completion summary instead of wrapping to the first; it reports reasoning patterns and links back to changed judgements without rewriting the original choice.
...
- **Training Availability:** Complete attributable observed ratings or complete source-matched expert grades and their verification milestone are required; calendar dates alone cannot enable it. Expert grades do not unlock rank-based browser options or Draft decisions.
...
- **Atlas Arrangement:** Colour is the default grouping, with Creatures/non-creatures, Rarity, None, and verified Tier available where supported. Keep one grouping level. Colour and tier sections retain jump navigation; other arrangements omit it. Sorting is independent of grouping.
...
Use labelled native controls on the active set’s paper surface. The first row narrows the cards by name, colour, type, and rarity. A separate row selects Group by and Sort by. Mana value is the initial sort when metadata exists; Name and verified Rating are alternatives. Type grouping separates creatures, non-creature spells, and lands, with plain headings and counts. None shows a continuous grid. Keep 44px minimum controls and a single card column on small screens. Hide controls and options whose metadata is unavailable. Offer Card size alongside arrangement: Compact keeps thumbnail rows; Large uses readable local images in four columns on desktop, two below 860px, and one below 700px, with the matching thumbnail as an error fallback. Remember card size locally across sets and keep it when clearing filters. Remember grouping and sorting locally; links preserve filters and arrangement. Clear filters restores all cards without changing their arrangement, returns focus to search, and result counts are announced. Colour filtering uses the same mutually exclusive colour categories as colour grouping, including Multicolour and Colourless.
