...
Training centres one readable card-and-answer object at a 1040px maximum, with browser-local progress inline below it. Expert review Training stacks its card and answer controls at 700px and below, centring the full card image at up to 244px wide and retaining visible meaning labels on the numerical buttons. Archetypes uses one broad format field note, a sticky route strip, and ruled full-width chapters rather than a grid of equal cards; each chapter keeps the plan and format read beside two recognisable signposts. Draft decisions places seat and historical-pool context in ruled binding rows, the complete pack in a seven-column map-leaf grid, a staged reason-and-lock step below the pack, and the revealed evidence in a flat three-column ledger followed by a keep/change reflection. All cards uses a single broad leaf, a compact colour/tier arrangement switch for rated sets, sticky navigation for the selected grouping, an auto-filling grid with a 230px minimum, and an in-place image lightbox. Major surfaces share a continuous binding/page shadow; rows remain flat and separated by rules.
...
- **Expert Choices and Answer:** Reality Fracture uses J2SJosh’s original 0–5 scale in eleven numerical choices with short meaning labels, arranged in three columns with a 64px minimum height. Existing violet accents and controls carry the choices without invented tier colours. Only after answering or revealing, show the target grade, both attributed reviewers’ grades, available teaching, and expandable source context. Keep original grades separate; do not average them or manufacture ranks or observed performance.
